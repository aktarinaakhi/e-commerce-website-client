...
const ToysAndSportProducts = () => {
    const allProduct = useSelector(state => state.products.allProducts);
    const toysANdSoprtProduct = allProduct.filter(baby => baby.category === 'toysAndSport');
    return (
        <div className="max-w-2xl mx-auto pt-16 px-4 lg:pb-1 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-1">
            <div className='mb-2'>
                <h2 className="text-2xl font-bold tracking-tight text-gray-900">Toys and Sport</h2>
            </div>
            ...
            <div className="mt-6 grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-4">
                {toysANdSoprtProduct.slice(0, 5).map((product) => (
...
export default ToysAndSportProducts;